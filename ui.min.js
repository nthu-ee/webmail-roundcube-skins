...
function rcube_elastic_ui() {
    var ref = this,
        mode = 'normal',
        // one of: large, normal, small, phone
    touch = false,
        ios = false,
        popups_close_lock,
    ...
    function setup() {
        ...
        if (!is_framed && layout.content.length && !$(layout.content).is('.no-navbar') && !$(layout.content).children('.frame-content').length) {
            env.frame_nav = $('<div class="footer menu toolbar content-frame-navigation hide-nav-buttons">').append($('<a class="button prev">').append($('<span class="inner"></span>').text(rcmail.gettext('previous')))).append($('<span class="buttons">')).append($('<a class="button next">').append($('<span class="inner"></span>').text(rcmail.gettext('next')))).appendTo(layout.content);
        }
        ...
        if (form = rcmail.gui_objects.messageform) {
            ...
            $('#compose-options').find('textarea,input,select').each(function () {
                var hidden = $('<input>').attr({ type: 'hidden', name: $(this).attr('name') }).appendTo(form);
    ...
    function create_cloned_button(target, menu_button, add_class, always_active) {
        ...
        button.attr({ id: button_id, href: '#', 'class': btn_class }).append($('<span class="inner">').text(target.text()));
    ...
    function init() {
        ...
        $('[data-list]').filter('ul,table').each(function () {
            ...
            if (rcmail[list] && rcmail[list].multiselect) {
                ...
                if (!button) {
                    button = $('<a>').attr({ 'class': 'button select disabled', role: 'button', title: rcmail.gettext('select') }).on('click', function () {
        ...
        if (window.MutationObserver) {
            $('[data-label-msg]').filter('ul,table').each(function () {
                var fn,
                    ...
                    if (!rcmail.env.search_request && !rcmail.env.qsearch && msg && !list.children(':visible').length) {
        ...
        if ((layout.list.length || layout.content.length) && is_mobile()) {
            ...
            $('[data-fab]').each(function () {
                ...
                if ((task == '*' || task == rcmail.task) && (action == '*' || action == rcmail.env.action || action == 'none' && !rcmail.env.action)) {
                    fabuttons.push(create_cloned_button(button, false, false, true));
                }
            });

            if (fabuttons.length) {
                $('<div class="floating-action-buttons">').append(fabuttons).appendTo(layout.list.length ? layout.list : layout.content);
            }
        }
    ...
    function bootstrap_style(context) {
        ...
        if (context != document && $('.popup', context).children().length == 1) {
            ...
            if (content.is('img')) {
                ...
                $('.popup', context).addClass('formcontent').append($('<div class="form-group row">').append(label.attr('for', id).addClass('col-sm-2 col-form-label')).append($('<div class="col-sm-10">').append(input)));
        ...
        $('form.tabbed,div.tabbed', context).each(function (idx, item) {
            ...
            $(this).addClass('tab-content').children('fieldset').each(function (i, fieldset) {
                ...
                tab = $('<li>').addClass('nav-item').append($('<a>').addClass('nav-link' + (tab_class ? ' ' + tab_class : '')).attr({ role: 'tab', 'href': '#' + id }).text($('legend', fieldset).first().text()).click(function (e) {
                    $(this).tab('show');
        ...
        // Make tables pretier
        $('table:not(.table,.compact-table,.propform,.listing,.ui-datepicker-calendar)', context).filter(function () {
        ...
        $('input.pretty-checkbox, .propform input[type=checkbox], .form-check input, .popupmenu.form input[type=checkbox], .menu input[type=checkbox]', context).each(function () {
            pretty_checkbox(this);
        });
        ...
        if (rcmail.env.task == 'login' && context == document) {
            ...
            $('#login-form table tr').each(function () {
                ...
                input.addClass(input.is('select') ? 'custom-select' : 'form-control').attr('placeholder', label.text()).before($('<span class="input-group-prepend">').append(icon)).parent().addClass('input-group input-group-lg');
            });
        }
    ...
    function tinymce_style(elem) {
        ...
        // TinyMCE menus on mobile
                ...
                $(elem).prepend($('<h3 class="popover-header">').append($('<a class="button icon "' + 'cancel' + '">').text(rcmail.gettext('close')).on('click', function () {
                    $(document.body).click();
                })));
                ...
                if (window.MutationObserver) {
                    var callback = function () {
                        ...
                        if (!$('.mce-menu:visible').length) {
                            ...
                            $('<div>').attr('class', 'popover-overlay mce-overlay').appendTo('body').click(function () {
                                $(this).remove();
                            });
                        }
                    };
    ...
    function content_frame_init() {
        ...
        rcmail.addEventListener('afterlist', list_handler).addEventListener('afterlistgroup', list_handler).addEventListener('afterlistsearch', list_handler)
        // plugins
                ...
                var overlay,
                    ...
                    $.each(data, function (i, v) {
                        ...
                        if (v.type == 'attributes') {
                            ...
                            if (is_hidden != hidden) {
                                if (!is_hidden) {
                                    overlay = $('<div>').attr('class', 'ui-widget-overlay datepicker').appendTo(win.document.body).click(function (e) {
                                        $(this).remove();
                                        if (is_framed) {
    ...
    function rcmail_popup_init(o) {
        ...
        if (is_mobile() && $(o.obj).is('.googie_window')) {
            ...
            var title = rcmail.gettext('close'),
                ...
                close_link = $('<a>').attr('class', class_name).text(title).click(function (e) {
                e.stopPropagation();
            ...
            if (!$('.popover-overlay').length) {
                $('<div>').attr('class', 'popover-overlay').appendTo('body').click(function () {
                    $(this).remove();
                });
            }
    ...
    function screen_resize() {
        ...
        switch (mode) {
            case 'phone':
                screen_resize_phone();break;
            case 'small':
                screen_resize_small();break;
            case 'normal':
                screen_resize_normal();break;
            case 'large':
                screen_resize_large();break;
        }
    ...
    function app_menu(show) {
        if (show) {
            if (mode == 'phone') {
                $('<div id="menu-overlay" class="popover-overlay">').on('click', function () {
                    app_menu(false);
                }).appendTo('body');
    ...
    function searchbar_init(bar) {
        ...
        rcmail.addEventListener('init', update_func).addEventListener('responsebeforesearch', update_func)
        // close options form on list/search request
        .addEventListener('beforelist', close_func).addEventListener('beforesearch', close_func);
    };
    ...
    function toolbar_init() {
        ...
        var list_mark,
            ...
            button.contents().filter(function () {
                if (this.nodeType == 3 && !$.trim(this.nodeValue).length) $(this).remove();
            });
        ...
        if (list_items.length) {
            var container = layout.list.children('.header'),
                ...
                menu_button = $('<a class="button icon toolbar-list-button" href="#list-menu">').attr({ 'data-popup': 'toolbar-list-menu' }),
        ...
        if (items.length) {
            var container = layout.content.children('.header'),
                ...
                menu_button = $('<a class="button icon toolbar-menu-button" href="#menu">').attr({ 'data-popup': 'toolbar-menu' });

            container
            // TODO: copy original toolbar attributes (class, role, aria-*)
            .append($('<ul>').attr(menu_attrs).data('popup-parent', container).append(items)).append(menu_button);
    ...
    function popup_init(item, win) {
        ...
        var level,
            ...
            popup = $(win.$('#' + popup_id).get(0)),
            // a "hack" to support elements in frames
        popup_orig = popup,
            ...
            if (win != window) {
                ...
                popup.attr('id', popup_id + '-clone').appendTo(document.body).find('li > a').attr('onclick', '').off('click').on('click', function (e) {
                    if (!$(this).is('.disabled')) {
                        $(item).popover('hide');
        ...
        $(item).attr({
            ...
            .find('[aria-haspopup="true"]').data('level', level + 1).off('click.popup').on('click.popup', function (e) {
                e.stopPropagation();
            });
            ...
            if (mobile) {
                ...
                $('.popover-header', popover).empty().append($('<a>').attr('class', class_name).text(title).on('click', function (e) {
                    $(item).popover('hide');
                    if (level > 1) {
                        e.stopPropagation();
                    }
    ...
    function spellmenu(obj) {
        ...
        if (!ul.length) {
            ...
            for (i in rcmail.env.spell_langs) {
                li = $('<li role="menuitem">');
                link = $('<a href="#' + i + '" tabindex="0"></a>').text(rcmail.env.spell_langs[i]).addClass('active').data('lang', i).on('click keypress', function (e) {
    ...
    function compose_status(id, status) {
        ...
        if (!status) {
            ...
            $('<a>').attr('class', 'button icon ' + id).on('click', function () {
                show_sidebar();
            }).appendTo(bar);
        }
    };
    ...
    function attachmentmenu_append(item) {
        ...
        if (!item.is('.no-menu') && !item.children('.drop').length) {
            ...
            }).append($('<span>').attr('class', 'inner').text(label)).appendTo(item);
        }
    };
    ...
    function headersmenu(obj, button, event) {
        $('li > a', obj).each(function () {
            ...
            link[$(target).is(':visible') ? 'removeClass' : 'addClass']('active').off().on('click', function () {
    ...
    function update_quota(p) {
        ...
        if (p.table) {
            element.css('cursor', 'pointer').data('popup-pos', 'top').off('click').on('click', function (e) {
    ...
    function recipient_input(obj) {
        var list,
            ...
            var recipient = $('<li class="recipient">'),
                name_element = $('<span class="name">').html(recipient_input_name(name || email)).on('dblclick', function (e) {
                recipient_input_edit_dialog(e, insert_recipient);
            }),
                email_element = $('<span class="email">'),

            // TODO: should the 'close' link have tabindex?
            link = $('<a>').attr({ 'class': 'button icon remove' }).click(function () {
                recipient.remove();
                apply_func();
                input.focus();
                return false;
            ...
            recipient.attr('title', name ? name + email : null).append([name_element, email_element, link]);
        ...
        input = $('<input>').attr({ type: 'text', tabindex: $(obj).attr('tabindex') }).on('paste change blur', parse_func).on('input', input_len_update) // only to fix input length after paste
        .on('keydown', keydown_func).on('focus mousedown', focus_func);

        list = $('<ul>').addClass('form-control recipient-input').append($('<li>').append(input)).on('click', function () {
            input.focus();
        });
        ...
        $(obj).css({ position: 'absolute', opacity: 0, left: '-5000px', width: '10px' }).attr('tabindex', -1).after(list)
    ...
    function image_upload_input(obj) {
        var reset_button = $('<a>').attr({ 'class': 'icon button delete', href: '#' }).click(function (e) {
    ...
    function iframe_loader(frame) {
        ...
        if (frame.length) {
            var loader = $('<div class="iframe-loader">').append($('<div class="spinner spinner-border" role="status">').append($('<span class="sr-only">').text(rcmail.gettext('loading'))));
    ...
    function pretty_checkbox(checkbox) {
        ...
        label.attr({ 'for': id, 'class': 'custom-control-label', title: checkbox.attr('title') || '' }).on('click', function (e) {
            e.stopPropagation();
        });

        checkbox.addClass('form-check-input custom-control-input').wrap('<div class="custom-control custom-switch">').parent().append(label);
    };
    ...
    function pretty_select(select) {
        ...
        var open_func = function (e) {
            ...
            $('option', select).each(function () {
                var label = $(this).text(),
                    link = $('<a href="#">').data('value', this.value).addClass(this.disabled ? 'disabled' : 'active' + (this.value == value ? ' selected' : ''));
            ...
            var list = $('<ul class="listing selectable iconized">').attr('data-ident', select_ident).data('button', select[0]).append(items).on('click', 'a.active', function () {
            ...
            select.popover('dispose').popover({
                ...
                offset: '0,2',
                sanitize: false,
                template: '<div class="popover select-menu" style="min-width: ' + min_width + 'px; max-width: ' + max_width + 'px">' + '<div class="popover-header"></div>' + '<div class="popover-body" style="max-height: ' + max_height + 'px"></div></div>'
            }).on('shown.bs.popover', function () {
                select.focus(); // for Chrome
                // Set popup Close title
                list.parent().prev().empty().append($('<a class="button icon cancel">').text(rcmail.gettext('close')).on('click', function (e) {
                    e.stopPropagation();
                    return close_func();
                }));
        ...
        select.addClass('pretty-select custom-select form-control').on('mousedown keydown', function (e) {
    ...
    function html_editor_init(obj) {
        ...
        var sw,
            ...
            tabs = $('<ul class="nav nav-tabs">').append($('<li class="nav-item">').append($('<a class="nav-link mode-html" href="#">').text(rcmail.gettext('htmltoggle')))).append($('<li class="nav-item">').append($('<a class="nav-link mode-plain" href="#">').text(rcmail.gettext('plaintoggle'))));
        ...
        $('a', tabs).attr('tabindex', tabindex).on('click', function (e) {
            var id = editor.attr('id'),
                is_html = $(this).is('.mode-html');
    ...
    function smart_field_row_add(area, value, name, idx, size, after) {
        ...
        var input,
            elem = $('<div class="input-group">' + '<input type="text" class="form-control">' + '<span class="input-group-append"><a class="icon reset input-group-text" href="#"></a></span>' + '</div>'),
        ...
        $(elem).find('input,a').on('focus', function () {
